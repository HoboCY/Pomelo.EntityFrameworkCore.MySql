...
SELECT COUNT(*)
FROM `JsonEntities` AS `j`
WHERE JSON_OVERLAPS(`j`.`CustomerJToken`, @element)
""");
        }
...
SELECT COUNT(*)
FROM `JsonEntities` AS `j`
WHERE JSON_OVERLAPS(JSON_EXTRACT(`j`.`CustomerJToken`, '$.Statistics.Nested.IntArray'), @element)
""");
        }
...
SELECT COUNT(*)
FROM `JsonEntities` AS `j`
WHERE JSON_CONTAINS(`j`.`CustomerJToken`, @element)
""");
        }
