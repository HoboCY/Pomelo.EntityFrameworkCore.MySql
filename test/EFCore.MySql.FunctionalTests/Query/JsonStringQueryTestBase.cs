...
""",
                //
                """
@expected='{"Name":"Joe","Age":25,"IsVip":false,"Statistics":{"Visits":4,"Purchases":3,"Nested":{"SomeProperty":10,"IntArray":[3,4]}},"Orders":[{"Price":99.5,"ShippingAddress":"Some address 1","ShippingDate":"2019-10-01"},{"Price":23,"ShippingAddress":"Some address 2","ShippingDate":"2019-10-10"}]}' (Size = 4000)

SELECT `j`.`Id`, `j`.`CustomerJson`
FROM `JsonEntities` AS `j`
WHERE `j`.`CustomerJson` = @expected
LIMIT 2
""");
        }
...
""",
                //
                """
@p='{"Name":"Joe","Age":25,"IsVip":false,"Statistics":{"Visits":4,"Purchases":3,"Nested":{"SomeProperty":10,"IntArray":[3,4]}},"Orders":[{"Price":99.5,"ShippingAddress":"Some address 1","ShippingDate":"2019-10-01"},{"Price":23,"ShippingAddress":"Some address 2","ShippingDate":"2019-10-10"}]}' (Size = 4000)

SELECT `j`.`Id`, `j`.`CustomerJson`
FROM `JsonEntities` AS `j`
WHERE `j`.`CustomerJson` = @p
LIMIT 2
""");
        }
